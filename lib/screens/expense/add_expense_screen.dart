...
class _AddExpenseScreenState extends ConsumerState<AddExpenseScreen> {
  ...
  void initState() {
    ...
    if (widget.prefilled != null) {
      _amountCtrl.text = widget.prefilled!['amount']?.toString() ?? '';
      _noteCtrl.text = widget.prefilled!['note'] ?? '';
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (_amountCtrl.text.isEmpty || _selectedCategory == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(l10n.pleaseEnterAmountAndCategory)),
      );
      return;
    }
  ...
  Future<void> _startListening() async {
    ...
    _speech.listen(
      onResult: (result) {
        ...
        // Parse "spent 200 on food" or "200 rupees"
        final amountMatch = RegExp(r'\d+(\.\d+)?').firstMatch(text);
        if (amountMatch != null) {
          _amountCtrl.text = amountMatch.group(0)!;
        }
        if (text.contains('food') || text.contains('restaurant') || text.contains('dining')) {
          _noteCtrl.text = 'Food';
        } else if (text.contains('transport') || text.contains('cab') || text.contains('uber')) {
          _noteCtrl.text = 'Transport';
        } else if (text.contains('shop') || text.contains('buy')) {
          _noteCtrl.text = 'Shopping';
        }
          ...
          IconButton(
            icon: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: _isListening ? AppColors.error.withOpacity(0.12) : AppColors.surfaceContainerHigh,
                shape: BoxShape.circle,
              ),
              child: Icon(
                _isListening ? Icons.mic : Icons.mic_none_rounded,
                color: _isListening ? AppColors.error : AppColors.outline,
                size: 18,
              ),
            ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                  colors: [AppColors.primaryContainer, Color(0xFF1565C0)],
            ...
            // Categories
            Text(l10n.category, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.onSurface)),
                ...
                child: _isSaving
                    ...
                    : Row(
                        ...
                        children: [
                          ...
                          const SizedBox(width: 8),
                          Text(l10n.saveExpense, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                        ],
                      ),
              ),
            ),
